...
Two ways out of a boundary that was meant to hold, and the documentation for
the one credential path that had none.

- **DuckDB is now inside the network policy.** Duckle's own HTTP client checks every host it dials, including each redirect hop, but SQL that DuckDB executes never passed through that client. `read_parquet` over https, a remote `ATTACH`, a `COPY` to a remote URI or an extension that fetches on its own could all reach a host the allowlist had refused. Scanning the SQL for those would not have closed it, because the SQL is generated by dbt, Python, templates and MCP, and a path can be built at run time from a row. So under `mode: enforce` with a `network.allowedDomains` list, a run now starts with DuckDB's remote filesystems disabled, community extensions refused and extension autoinstall off. DuckDB will not re-enable a disabled filesystem or community extensions in a running database, which is what makes it a boundary rather than advice: a later statement cannot lift it, whoever wrote that statement. Local file access is untouched, and `network.allowDuckdbExternalIo` returns DuckDB-native remote reads to an operator who needs them, from the server policy alone.
- **The MCP server resolves vault and environment placeholders.** Every other way to run a pipeline (the desktop app, the scheduler, `duckle-runner run`, `serve`, `follow`) resolves `${VAULT:...}` and `${ENV:...}` before the document reaches the engine. The MCP server did not, so a credential held in a vault arrived at the connector as that literal text and the connection failed with a password that was never fetched. The server's own tool instructions tell an agent to write `${ENV:KEY}` rather than a literal, so it had been advertising a mechanism it did not honour.
- **The vault tier is documented.** `${VAULT:NAME}` fetches a credential at run time from whatever the host's `DUCKLE_VAULT_COMMAND` names, so it never lands in a workspace, an environment variable or an image layer. It had no entry in the secrets guide despite being the only tier that keeps a credential out of all three. There are now worked examples for CyberArk AIM and Conjur, HashiCorp Vault and AWS Secrets Manager, along with what happens when the command fails, why the command is host configuration a pipeline can never name, and the fact that rotating a vaulted secret needs nothing on the Duckle side at all.
- **Security, governance and compliance documentation.** A documentation set covering the trust boundary and threat model, the encryption and key hierarchy, secrets management, audit logging and SIEM integration, incident handling, vulnerability reporting, and a SOC 2 and ISO 27001 control matrix. Contributed by @dydek in #294.
